        ...
        <div class="bg-light lter b-b wrapper-md">
            <div class="row">
              ...
              <div class="col-sm-6 col-xs-12 text-right hidden-xs">
                  <h1 class="m-n font-thin h3 text-black font-bold">Campaign's Real-Time Report</h1>
...
<script>
  ...
  $(document).ready(function() {
    ...
    var remainingContacts = {{ $campaign->total_numbers }};
    ...
    function livecheck() {
       $.ajax({
        ...
        success: function(res) {
          ...
          campaign_status = res.campaign_status;
    ...
    //live tracking
    setInterval( function () {
      if (remainingContacts!=0 || campaign_status!="Complete") {
    ...
    $("#forceRefund").click(function() {
        var txt;
        var r = confirm("Are you sure? It will give refund for the failed/pending sms and will mark the campaign as complete");
        if (r == true) {
          $(this).attr('disabled', true);

          $.ajax({
            url: "{{ config('apiconfig.api_url') }}/sms-api/process-forcerefund",
            type: "post",
            data: {
              api_token: '{{ $api_token }}',
              campaign: {{ $campaign->id }}
