...
*/
    ...
    public function contactGroupNumbers($id)
    {
        ...
        $sql = "SELECT contacts.id, contacts.contact_number, contacts.contact_name, contacts.email, contacts.gender, contacts.dob FROM contacts WHERE contact_group_id=$id ";
    ...
    public function deleteMultipleContactInfo(Request $request)
    {
        $ids = explode(',', $request->delete_data);
        Contact::whereIn('id',$ids)->where('user_id',Auth::guard('web')->user()->id)->delete(); 
        return redirect()->back()->with('success', 'Information Deleted Successfully');  
    }

    public function deleteContactInfo(Request $request)
    {
        Contact::where('id',$request->id)->where('user_id',Auth::guard('web')->user()->id)->delete();
        return redirect()->back()->with('success', 'Information Deleted Successfully');  
    }

    public function updateContactInfo(Request $request)
    {
       Contact::where('id',$request->id)->where('user_id',Auth::guard('web')->user()->id)->update($request->except(['id','_token']));
